...
| `torch.cuda.is_available()` returns False | Template lacks CUDA PyTorch. Verify with `python -c "import torch; print(torch.__version__)"` — needs `+cu` suffix. If missing: `pip install torch --index-url https://download.pytorch.org/whl/cu126`. |
| `RuntimeError: The NVIDIA driver on your system is too old (found version XXXXX)` | PyTorch wheel was built for newer CUDA than the host driver supports. Check the listing's "Max CUDA" field — if it's 12.8, use cu128; if 12.4, use cu124; etc. Fix: `pip install --force-reinstall torch --index-url https://download.pytorch.org/whl/cu128`. CUDA is backward-compatible (newer driver runs older code) but not forward-compatible — match PyTorch's CUDA to the *driver*, not the GPU architecture. |
| `... is of compute capability (CC) 12.0` warning + `is not compatible with the current PyTorch installation` | The PyTorch wheel doesn't include kernels for Blackwell GPUs (sm_120). Older wheels (cu118/121/124/126) only support up to sm_90 (Hopper). For Blackwell, you need cu128: `pip install --force-reinstall torch --index-url https://download.pytorch.org/whl/cu128` (or `nightly/cu128` if not on stable yet). Note: `is_available()` may return True even with this warning, but actual CUDA ops will fail. |
